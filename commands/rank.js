...
module.exports.run = async (client, message, args) => {
  ...
  let level = client.db.get(`${message.guild.id}level_${user.id}`) || 0;
  let exp = client.db.get(`${message.guild.id}xp_${user.id}`) || 0;
  let neededXP = Math.floor(Math.pow(level / 0.1, 2));

let every = client.db.all().filter(i => i.ID.startsWith(`${message.guild.id}xp_`)).sort((a, b) => b.data - a.data);
  let rank = every.map(x => x.ID).indexOf(`${message.guild.id}xp_${user.id}`) + 1;
